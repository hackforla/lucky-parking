...
[parking citations dataset](https://data.lacity.org/Transportation/Parking-Citations/4f5p-udkv) (~6 GB CSV, millions of
rows).
...
PostGIS adds a **load → clean pipeline** (`parking_pipeline.py`) that builds a slim `citations_clean` table after each
full CSV load.

For module-by-module code breakdown, schemas, and data-flow diagrams, see **[ARCHITECTURE.md](ARCHITECTURE.md)**.
...
Streams the CSV in 100k-row Polars batches — the full file is never loaded into memory. Normalizes `issue_date` to
ISO 8601. Safe to re-run (`INSERT OR IGNORE`).
...
Fetches from the Socrata API newest-first and stops when a page contains a `ticket_number` already in the DB.
...
PostGIS API sync is not implemented yet — use SQLite `sync` today, or extend `parking_postgis.py` with the same Socrata
loop from `parking_db.py`.
...
- **Sync uses `INSERT OR IGNORE`** — corrections to existing tickets are not applied. See
