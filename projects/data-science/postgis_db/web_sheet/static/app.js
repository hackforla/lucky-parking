(() => {
	...
	function wireSuggest(wrap) {
		...
		async function fetchSuggestions(q) {
			...
			activeIndex = -1;
			if (!items.length) {
				renderSuggestionMessage(q ? "No matching regions" : "No regions available for this type");
				return;
			}
	...
	const suggestControls = [...document.querySelectorAll("[data-suggest]")].map(wireSuggest);
	...
	function initMap(mapEl) {
		if (!mapEl) return;
		const source = mapEl.parentElement && mapEl.parentElement.querySelector("[data-map-markers]");

		if (!window.L) {
			mapEl.innerHTML = '<p class="map-empty">Map library failed to load. Hard-refresh the page.</p>';
			return;
		}

		let markers = [];
		try {
			markers = JSON.parse(source ? source.textContent : "[]").filter((r) => r.lat != null && r.lon != null);
		} catch (_) {
			mapEl.innerHTML = '<p class="map-empty">Could not parse citation coordinates.</p>';
			return;
		}

		const map = L.map(mapEl, { scrollWheelZoom: true }).setView([34.05, -118.25], 11);
		...
		markers.forEach((r) => {
			const fine = r.fine_amount == null ? "" : `$${Number(r.fine_amount).toFixed(2)}`;
			const popup = [
