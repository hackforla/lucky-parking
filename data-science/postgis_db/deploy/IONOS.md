# Deploy Lucky Parking to IONOS (US) — DB + API + explorer UI

Target: **VPS M+** (4 GB RAM, 120 GB NVMe) — ~$4/mo intro on [IONOS VPS](https://www.ionos.com/servers/vps). Budget **~$8–11/mo** after renewal.
...
1. Order **VPS M+** (Ubuntu 24.04 LTS, US datacenter).
...
- [ ] Optional: IONOS Cloud Backup for the `postgis_data` volume
...
| OOM on 2 GB S+ | Use M+ or lower `COMPOSE_MEM_LIMIT_*` and run API **or** web, not both |
| Init + restore conflict | Keep `SKIP_CITATIONS_LOAD=1`; restore full dump after boundary init |

Local dev unchanged: use `docker compose up` (dev file publishes :5432 for tools).
